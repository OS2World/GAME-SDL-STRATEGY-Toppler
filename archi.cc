...
archive::archive(FILE *stream) : f(stream) {
  ...
  assert(f, _("Data file not found"));
  ...
  assert(files, _("Failed to alloc memory for archive index."));
  ...
  for (Uint8 file = 0; file < filecount; file++) {
    ...
    /* find ut the length of the name string */
    {
      ...
      do {
        strlen++;
        assert(strlen <= FNAMELEN, _("Filename too long, datafile corrupt?"));
        fread(&c, 1, 1, f);
      } while (c);
...
file::file(const archive *arc, const char *name) : bufferpos(0) {
  ...
  for (Uint8 i = 0; i < arc->filecount; i++) {
    if (strncmp(name, arc->files[i].name, FNAMELEN) == 0) {
      ...
      assert(uncompress(buffer, &fsize, b, arc->files[i].compress) == Z_OK, _("Decompression problem, data file corrupt?"));
      assert(fsize == arc->files[i].size, _("Data file corrupt."));
